...
including RDW/SDW and RPTR). When lengthening a record, subtract the difference;
...
When a file can no longer be extended a secondary file needs to be allocated. When that is done transparently,
`CTRNEXT` needs to be incremented. Setting its initial value is a function of zRepro.
...
When adding a record the record length (including SDW/RDW, but excluding RPTR) needs to be added to
...
The prefix counter field `CTRLOKEY@` points to a value of `PFXKYLEN` bytes.
...
When erasing a record the record length (including SDW/RDW, but excluding RPTR) needs to be subtracted
