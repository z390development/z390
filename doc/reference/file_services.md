...
!!! Note "VSAM support"
    ...
    to [VSAM file services](vsam_file_services.md).
...
In order to OPEN a file, the DDNAME specified on the DCB must be connected
to an actual file. This can be done in two ways:
1. Specify DSNAME= instead of DDNAME= on the DCB. This creates a hard-coded link to the file named in DSNAME=
2. Specify an environment variable for the DDNAME specified on the DCB. Do not specify the DSNAME on the DCB.
   This is the preferred approach - it allows the program to access different physical files as needed
   without having to modify/assemble/link the program.

The content of the environment variable for QSAM/BSAM files must adhere to the following syntax rules:
- content may be an absolute or relative path, with filename and extension specified explicitly
- path separators can be forward or backward slashes, z390 adjusts for the OS reuirements
- only for SORTIN: the file specification may be followed by `[RECFM=xx]` to specify the record format of the input data

The content of the environment variable for VSAM files must adhere to the following syntax rules:
- content may be an absolute or relative path to the catalog module that defines the cluster
- instead of the `.390` extension the period must be followed by the catalog entry name defining the file to be opened
- path separators can be forward or backward slashes, z390 adjusts for the OS reuirements
- it is possible to define a CATALOG environment variable, but this variable is not used by zVSAM.
  If defined, it serves as documentation only.
...
DD statement idenrtifier and Dataset name respectively.
...
- MACRF=PM
    - Records are written LRECL at a time into a block of size BLKSIZE. CLOSE may write a short block.
- MACRF=W or RW
