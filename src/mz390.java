...
public  class  mz390 {
	 ...
	 *                           4. Add tz390 macro APARM generated by ACALL to reset parm just before AENTRY
	...
	private void load_mac(){
		 ...
		 *     b.  ACALL, AENTRY, AEXIT. AEND // RPI 2220 add APARM
		 *     c.  AWHILE, AUNTIL, AEXIT, AEND
		 *     d.  ACASE, AWHEN, AELSE, AEXIT, AEND
		 *     e.  Note ACALL generates APARM macro call to reset parms for AENTRY  RPI 2220
		 *
		 * Notes:
	...
	private boolean zsm_find_name(){
		...
		if (zsm_acall_index >= 0){
			...
			log_error(280,"zsm_find_name hash index duplicate for - '" + zsm_acall_name[zsm_acall_index] + "' and '" + acall_name + "'");
			return false;
		}
	...
	private boolean find_var(String name){
	    ...
	    String zsm_hash_key = "N" + tot_macros + var_name.substring(1);         // RPI 2220 is this acall name within same macro
		zsm_acall_index = tz390.find_key_index('Z',zsm_hash_key);  // RPI 977
		if (zsm_acall_index >= 0 
		 && zsm_acall_name[zsm_acall_index].equals(var_name.substring(1))){
			var_type = var_sublist_type;
			var_loc = var_sysalist_loc;
			var_name_index = -1;
			return true;
		}
		/*
	...
	private void exec_mac_op(){
		...
		case 225:  // OPSYN
			...
			break;
		case 228: // APARM RPI 2220 see tz390 op_table_directives add for APARM maccro
		    zsm_aparm_tot++;
		    if (!zsm_find_name()){
				log_error(249,"ZSM APARM name error - " + tz390.split_parms);
				return;
			}
			zsm_acall_parm[zsm_acall_index] = acall_parm; // reset acall parm to current aparm set by zsm_find_name just before aentry
			bal_op_ok = true;
		    break;
		default: 
			tz390.abort_case();
