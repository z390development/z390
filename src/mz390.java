...
public  class  mz390 {
	...
	private void exec_aif(){
		...
		while (aif_test_index >= 0){
			...
			if (setb_value != 0 
				...
				actr_count--;
				if (new_mac_line_index + 1 < mac_name_line_start[mac_name_index] // rpi 899 add +1
	...
	private boolean inc_tot_exp_stk_var(){
		...
		if (tot_exp_stk_var < max_exp_stk){
			exp_var_pushed = true;
			tot_exp_stk_var++;
			exp_stk_var_name_index[tot_exp_stk_var-1] = -1; // RP 833
			return true;
		} else {
	...
	private void put_listcall(){
		 ...
		 */
			...
			String sysndx = tz390.right_justify("" + (lcl_sysndx+1),4);
    	...
    	if (tot_exp_stk_var > 0){
			...
			case 23: // setc
				if (var_loc == var_lcl_loc){ // RPI 447 was var_loc
					...
					if (var_name_index >= 0 // RPI 901
						&& gbl_set_end[var_name_index] - gbl_set_start[var_name_index] > 1
						&& gbl_set_high[var_name_index] > 0){
