 ...
 * 2020-12-18 dsh rpi 2220 add support for ACALL argument sublist
 *                           1. Add arguments in parm after ACALL NAME(P1,P2,,,)  Example ACALL SUB1(A,B)
 ^                           2. Access parms via &NAME(sub1,sub2)                 Example N'&SUB1 = 2, &SUB1(2) = B
 *                           3. Use tot_macros to make ACALL and AENTRY hash key unique within a macro
 *                           4. Add tz390 opcode APARM generated by ACALL to reset parm just before AENTRY
...
public  class  mz390 {
        ...
        while (mac_line != null
            ...
            if (mac_op != null && mac_op.length() > 0) {
                if (mac_op.equals("MACRO")) {
                    tot_macros++; // rpi 2220 count macros - used for unique acall keys
        ...
        try {
            ...
            if  (temp_line == null) {
                mac_line = null;
                    ...
                    || temp_line.charAt(mac_ictl_end[cur_mac_file]) <= asc_space_char) { // RPI 728 test cont col RPI181
                ...
                while (mac_cont_line) { //RPI181 //RPI 215
                    ...
                    if  (temp_line.length() >= mac_ictl_start[cur_mac_file] + mac_ictl_cont[cur_mac_file] - 1 // RPI 728
                            && temp_line.substring(mac_ictl_start[cur_mac_file]-1,mac_ictl_cont[cur_mac_file]-1).trim().equals("") ) { // RPI 728 check all spaces on preceeding cont // RPI 167
    ...
    private void zsm_gen_lines() {
        ...
        case 7: // ACALL
            ...
            insert_acall_parms(); // #335 set &(acall_name)(1-n) to acall_parm(1-n)
    ...
    private boolean zsm_find_acall_name() {
        ...
        int index = acall_name.indexOf(' ');
        if (index > 0)acall_name = acall_name.substring(0,index); // remove comments
        acall_parm = "";
        index = acall_name.indexOf('('); // rpi 2220 replace space with (
        if (index > 0) {
            acall_parm = acall_name.substring(index);
            acall_name = acall_name.substring(0,index); // remove parms from name(...)
        }
        String zsm_hash_key = "N" + tot_macros + acall_name; // rpi 2220 make acall unique to macro
        zsm_acall_index = tz390.find_key_index('Z',zsm_hash_key);  // RPI 977
        if (zsm_acall_index >= 0) {
            if (zsm_acall_name[zsm_acall_index].equals(acall_name)) {
                if (acall_parm.length() > 0)zsm_acall_parm[zsm_acall_index] = acall_parm;
                return true;
            }
            log_error(280,"zsm_find_acall_name hash index duplicate for - '" + zsm_acall_name[zsm_acall_index] + "' and '" + acall_name + "'");
            return false;
        }
        if (zsm_aentry_name_tot < max_zsm_aentry_name) {
            zsm_acall_index = zsm_aentry_name_tot;
            if (!tz390.add_key_index(zsm_acall_index)) {
                abort_error(281,"zsm_find_acall_name hash table error");
                return false;
            }
            zsm_aentry_name_tot++;
            zsm_acall_name[zsm_acall_index] = acall_name;
            zsm_acall_cnt[zsm_acall_index] = 0;
            zsm_acall_parm[zsm_acall_index] = acall_parm; // rpi 2220 store parms (p1,p2,,,) from acall
    ...
    private void insert_acall_parms() {  // #335
        ...
        int acall_arg_num = 1;
        acall_arg_comma = acall_arg.indexOf(',');
        while (acall_arg_comma > 0) {
            if (!acall_arg.substring(0,1).equals("\'")) {  // #335
                acall_arg_insert = "&(" + acall_name + ")(" + acall_arg_num + ") SETC \'" + acall_arg.substring(0,acall_arg_comma) + "\'";
            } else {
                acall_arg_insert = "&(" + acall_name + ")(" + acall_arg_num + ") SETC " + acall_arg.substring(0,acall_arg_comma);
            }
            acall_arg_num++;
            insert_acall_set(acall_arg_insert);
            acall_arg = acall_arg.substring(acall_arg_comma+1);
            acall_arg_comma = acall_arg.indexOf(',');
        }
        if (acall_arg.length() > 0) {
            if (!acall_arg.substring(0,1).equals("\'")) {  // #335
                acall_arg_insert = "&(" + acall_name + ")(" + acall_arg_num + ") SETC \'" + acall_arg + "\'";
            } else {
                acall_arg_insert = "&(" + acall_name + ")(" + acall_arg_num + ") SETC " + acall_arg;
            }
            insert_acall_set(acall_arg_insert);
    ...
    private String replace_vars(String text,boolean reduce,boolean check_label) {
        ...
        while (var_match.find()) {
            ...
            if (parm_value.length() == 1) {
                ...
                if (parm_value == null
                    && bal_text.length() >= bal_text_index1 + 8    // RPI 2208
            ...
            if (!parse_set_var(text,index)
                ...
                log_error(107,"set type conflict for - " + text.substring(index));
                       ...
                       && (exp_parse_set_name.length() <= 4
                           || !exp_parse_set_name.substring(0,4).toUpperCase().equals("&SYS")
                          )
                       ) { // RPI 1139
    ...
    private void exp_calc_var_sub() {
        ...
        if (tot_exp_stk_var >= 2) {
            ...
            if (exp_stk_var_type[tot_exp_stk_var - 2] == var_subscript_type) {
                ...
                case 16: // sysalist(sub) or sysalist(sub, // RPI 2220 where sysalist is name of acall routine
                    var_type = var_sublist_type;
                    var_name = "&SYSLIST";
                    if  (exp_next_first == ')') {
                        exp_level--;
    ...
    private boolean is32BitBinaryInteger(String s, int base) {             // #509
        ...
        case 10:
            ...
            try {
                if (Long.parseUnsignedLong(s) >= Integer.MAX_VALUE + 1L) return false;
    ...
    private int get_int_from_string(String setc_text,int base) {
        if (!tz390.opt_allow && base == 10
            ...
            if (first < '0' || first > '9') {
                // rpi 2210 support &var seta &varname such as n equ 25
    ...
    private int mz390_find_sym(String symbol) {
        ...
        if (!tz390.opt_asm) { // RPI 427
            // RPI 2228 for ZPARTRS log_error(190,"ordinary symbol reference requires ASM option - " + symbol);
            return -1;
    ...
    private void init_gbl_sys() {
        ...
        add_gbl_sys("&SYSZVSAM",var_seta_type);      // rpi 1628 add option zvsam and gbla &syszvsam
        gbl_seta[tot_gbl_seta-1] = tz390.opt_zvsam;  // rpi 1628
        if (tz390.opt_bs2000) {  // RPI 604
    ...
    private synchronized void abort_error(int error,String msg) {
        ...
        if (mac_file_num != null && mac_line_index >= 0) { // RPI 812 RPI 1052
            ...
            if (mac_line_index < tz390.opt_maxline) {
                err_line_and_num = " file=" + (file_index+1)
    ...
    private void gen_pc(byte op) {
        ...
        case  3: // gen pc_op_pushv  push var on stack
            ...
            mz390_find_sym(var_name);   // force var lookup via az390
        ...
        case  4: // gen pc_op_pushvs       calculate var subscript
            ...
            pc_var_loc[pc_loc]  = var_loc;
            if (var_name.equals("&SYSLIST")) {
            // #335 || var_loc == var_sysalist_loc) { // rpi 2220 replaced by created sym
                pc_seta[pc_loc] = -1;
            } else {
                pc_seta[pc_loc] = var_name_index;
            }
    ...
    //#318        if (setc_value.length() > 3  issue #318 let az390 convert sdts to numeric values              // rpi 2217 is this sdt of the form ?'...' fir seta value
     //#318         && setc_value.charAt(1) == '\''
    //#318         && setc_value.charAt(setc_value.length()-1) == '\'') {
    //#318            tot_exp_stk_var -=1;               // rpi 2017 backup for push_sdt inc
    //#318            exp_push_sdt();                    // RPI 2017 push sdt seta value
    //#318           return;
    //#318        } else {
            exp_stk_val_type[tot_exp_stk_var - 1] = val_setc_type; // RPI 447
            exp_stk_setc[tot_exp_stk_var - 1] = setc_value;
    ...
    private void exec_pc_pushvs() {
        if (inc_tot_exp_stk_var()) {
            ...
            case 14: // kw  parm var(sub) or var(sub,
                ...
                break;
            case 15: // syslist(sub) or syslist(sub,
            // #335 case 16: // sysalist(sub) rpi 2220 replaced by created sym
                var_type = var_parm_type;
                val_type = val_setc_type;
                if (var_name_index == -1) {
                    if (   zsm_acall_index >= 0
                        && var_loc == var_sysalist_loc) { // rpi 2220 use acall parm
                        setc_value = zsm_acall_parm[zsm_acall_index];
                        if (set_sub > 0) {
                            setc_value = get_sublist(setc_value,set_sub);
                        }
                        exp_stk_setc[tot_exp_stk_var-1] = setc_value;
                        if (tz390.opt_traceall) {
                            tz390.put_trace("SYSALIST PARM ='" + setc_value + "'");
                        }
                    } else if (mac_call_level > 0 && set_sub >= 0) {
    ...
    private boolean get_pc_var() {
        ...
        if (pc_sysndx[pc_loc] != mac_call_sysndx[mac_call_level]) {
            ...
            if (var_loc != var_gbl_loc
                && var_loc != var_syslist_loc
                && var_loc != var_sysalist_loc) {  // rpi 2220
                switch (var_loc) {
                case 11: // lcl set
        ...
        case 16: // sysalist parm rpi 2220
            var_type = var_parm_type;
            val_type = val_setc_type;
            break;
        default:
        ...
        if (tot_exp_stk_var > 0) {
            ...
            case 26: // sublist var_sublist_type
                if (var_loc == var_syslist_loc      // RPI 447 was setb
                    ...
                    put_seta_stack_var();
                } else if (var_loc == var_sysalist_loc
                        && zsm_acall_index >= 0) { // RPI 2220
                    // sysalist acall parm sublist count
                    setc_value1 = zsm_acall_parm[zsm_acall_index];
                    seta_value = get_sublist_count(setc_value1);
                    tot_exp_stk_var--; // remove sysalist var
    ...
    private void exec_pc_pushs() {
        ...
        if (index >= 0) {
            seta_value = az390.sym_loc[index];
            if (tz390.opt_traceall) {
                tz390.put_trace("pushing sym " + setc_value + " = " + seta_value); // rpi 2210
            }
            put_seta_stack_var();
