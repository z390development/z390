...
public class sz390 implements Runnable {
    ...
    /** variable      */ int ascii_period =  '.';
    /** variable      */ int ascii_space = ' ';
    ...
    /*
     * end of global sz390 class data and start of procs
     */
    ...
    public synchronized void abort_error(int error,String msg) {  // RPI 646
        ...
        if (ez390_recursive_abort) { // RPI 935
            System.out.println("EZ390E abort recursive exit");
            System.exit(16);
        }
    ...
    private void put_stats() {
        ...
        if (tz390.opt_timing) { // display instr rate
            ...
            long ins_rate = (tz390.systerm_ins)*1000/(tod_end_pgm - tod_start_pgm + 1);
    ...
    public void init_time() {
        ...
        pz390.ibm_mil = - cur_date_cal.getTime().getTime(); // milliseconds from 1900 to 1970
    ...
    public void svc_load() {
        ...
        if (cur_cde != -1 && cde_loc[cur_cde] != 0) {
            if ( cde_use[cur_cde] < 0x7FFF ) {
                cde_use[cur_cde]++;                                                 // RPI 1063 no overflow   #658
            }
            ...
            while (cur_rld < load_code_rlds) {
                ...
                case 2: // RPI 894
                    ...
                    if (rld_field > 0xffff) { // RPI 894
                        abort_error(120,"loader 2 byte RLD address "
                                        + tz390.get_hex(rld_loc,8)
                                        + " too high in - "
                                        + tz390.pgm_name); // RPI 2225
                    }
                    pz390.mem.position(load_code_load + rld_loc);
                ...
                case 3: //RPI71
                    ...
                    if (rld_field > 0xffffff) { // RPI 894
                        abort_error(121,"loader 3 byte RLD address too high in - "
                                        + tz390.pgm_name);
                    }
                    pz390.mem.position(load_code_load + rld_loc);
                ...
                case 8: // RPI 270
                    ...
                    break;
                default:
                    abort_error(119,"invalid RLD length in load module - "
                                    + tz390.pgm_name
                                    + " at offset "
                                    + tz390.get_hex(rld_loc,8)); // RPI 2225
                }
                cur_rld++;
            }
        } catch (Exception e) {
    ...
    /**
     * dump regs and optionally dump everything
     *
    ...
    public void dump_fpcplus(int fpc) {                                          // #515
        ...
        int rm = (fpcRoundingMode & 0x00000070) >>> 4;                           // #515
        ...
        tz390.put_trace("     Value(bin)  Rounding Method");                     // #515
        rm = fpcRoundingMode & 0x00000007;                                       // #515
        sVal = Integer.toBinaryString(rm+8).substring(1);                        // #515
    ...
    private void dump_mem_stat() {
        tz390.put_trace(" MEM TOTAL=" + (pz390.tot_mem >> 20) + "MB"
                        + "  ALLOC=" + pz390.tot_mem_alloc
    ...
    public synchronized String cmd_get_queue(int cmd_id) {
        ...
        try {
            String cmd_output_line = cmd_output_queue[cmd_id].remove();
            tot_log_queue--;
     ...
     *    <li>x'2000' - dump fpr f0-f15 and fpc</li>
    ...
    private void exec_test_cmd() {
        try { // RPI 1137
            if (test_cmd != null && test_cmd.length() > 0) {
                ...
                if (test_cmd.charAt(0) == '*') {
                    return; // ignore comment lines // #853
                }
     ...
     * and just issue invalid int error if error
     * and return -1.
     *
    ...
    private void zsort_put() {
        ...
        zsort_move_rec(pz390.reg.getInt(pz390.r1), zsort_blk_ptr); // move rec to blk
        if (tz390.opt_traceall) {
    ...
    private boolean zsort_comp(int rec1,int rec2) {
        ...
        while (key_index < zsort_tot_keys) {
            ...
            if (tz390.opt_traceall) {
                tz390.put_trace(
                                "ZSORT COMPARE KEYS  OFF="
                                + zsort_key_off[key_index]
                                + " LEN=" + key_len
