...
public  class  pz390 {
   /*****************************************************
    ...
    sz390 svc component to perform os functions such as memory allocation and loading.
    ...
    *          reg.get and reg.put with direct byte array. 
    ...
    * 01/04/06 RPI107 split ez390 into ez390, pz390
    * 01/06/06 RPI158 clear remainder of register for LL????
    * 01/08/06 RPI160 correct LLGT loading from loc + 4
    * 01/12/06 RPI 151 add LPSW support as branch.
    * 01/15/06 RPI 173 add LRV, STPQ, LPQ  support.
    * 01/26/06 RPI 172 move options to tz390, svc > sz390
    * 02/02/06 RPI 175 rewrite AL??? SL??? using long vs bigint
    * 02/02/06 RPI 185 add z9 opcodes AFI, AGFI, etc.
    * 02/04/06 RPI 197 remove test_or_trace interrupts
    * 02/08/06 RPI 185 add STCKF and fix STCK to be unique
    ...
    * 04/10/06 RPI 276 add zcvt_user_pgm
    * 04/12/06 RPI 244 support ESPIE/ESTAE PARAM, CT,OV
    * 04/21/06 RPI 279 stimer real exit support
    * 04/23/06 RPI 295 correct ICM, ICMH, ICMY,
    *          OIHL, and OILH CC values.
    * 04/26/06 RPI 299 check for 0C5 in setups
    * 04/27/06 RPI 298 add MAYLR MYLR MAYHR
    *          MYHR, MAYL MYL MAYH MYH
    * 04/28/06 RPI 301 force 0C7 on PD zero sign was checking
    *          was checking first non-zero byte beyond in error.
    * 05/02/06 RPI 305 update ESPIE and ESTAE support and
    *          fix get_pd for pd instr. to avoid
    *          recursive program checks
    * 05/02/06 RPI 307 correct MAYL and MAYLR to remove duplicate
    *          setup call corrupting next instruction, and correct
    *          IPM trace format.
    * 06/03/06 RPI 323 allow opcode break on first stimer exit instr.
    * 06/03/06 RPI 325 allow KEB and KDD exact 0
    * 07/03/06 RPI 326 add TCEB, TCDB, and TCXB test data class
    * 07/03/06 RPI 333 add SRNM and support rounding modes
   ...
   */ 
    ...
    long long_zone_ones = (((long)0xf0f0f0f0) << 32) | ((long)0xf0f0f0f0 & long_low32_bits);
    ...
    int[] espie_pie   = (int[])Array.newInstance(int.class,max_espie); // espie psw_pic bit mask from R0
    int[] espie_exit  = (int[])Array.newInstance(int.class,max_espie); // espie exit addr from R1
    int[] espie_parm = (int[])Array.newInstance(int.class,max_espie); // parm address if not zero from R15
    int        max_estae   = 50;
    int        tot_estae    = 0;
    ...
    byte  fp_ctl_eb = 1;
    byte  fp_ctl_db = 2;
    byte  fp_ctl_bd1 = 3; // first  half lb/lh
    byte  fp_ctl_bd2 = 4; // second half lb/lb at +2
    ...
    MathContext fp_e_context = MathContext.DECIMAL32;  // fp_fpc_rnd default half even
    MathContext fp_d_context = MathContext.DECIMAL64;  // fp_fpc_rnd default half even
    MathContext fp_x_context = MathContext.DECIMAL128; // fp_fpc_rnd default half even
     ...
     */      
        ...
        int zcvt_tget_ecb    = zcvt_start+0x1c;  // ecb for tget reply in non gui mode
        int zcvt_save        = zcvt_start+0x100;       // user save  
        int zcvt_stimer_save = zcvt_start+0x200;
        int zcvt_exec_parm   = zcvt_start+0x300;  // half word length followed by EBCDIC/ASCII value of PARM(.)
		...
		switch (opcode1){
		 case 0x01:  
		     ...
		     case 0x04:  // 30 "0104" "PTFF" "E" Z9-1
		    	  ...
		    	  * Notes:
		    	  *   1.  Map functions 0x00-0x43 to 
		    	  *       timing functions 0x80-C3 in svc 11
		    	  */
		         psw_check = false;
		    	 ins_setup_e();
		 ...
		 case 0x42:  // 880 "42" "STC" "RX"
		 	 psw_check = false;
		     ins_setup_rx();
		     mem_byte[xbd2_loc] = reg_byte[rf1+4+3];
		     break;
		 case 0x43:  // 890 "43" "IC" "RX"
		 	 psw_check = false;
		     ins_setup_rx();
		     reg_byte[rf1+7] = mem_byte[xbd2_loc];
		     break;
		 case 0x44:  // 900 "44" "EX" "RX"
		     ...
		     if (!ex_mode){
		         ...
		         if (rf1 != 0){
		         	mem_byte[psw_loc+1] = (byte)(ex_mod_byte | reg_byte[rf1+7]);
		         }
		     } else {
		 ...
		 case 0xB2:  
		     ...
		     case 0x05:  // 2590 "B205" "STCK" "S"
		          ...
		          *       since Jan. 1, 1970.
		          *    2. For IBM OS compatibility the STCK 
		          *       instruction converts Java time to 
		          *       base of January 1, 1900 with bit 51
		          *       being incremented each microsecond.
		          *    3. The low bits are incremented to prevent
		     ...
		     case 0x78:  // 3270 "B278" "STCKE" "S"
		          ...
		          *   1.  The Java time stored is milliseconds
		          *       since Jan. 1, 1970.
		          *    2. For IBM OS compatibility the STCKE 
		          *       instruction converts Java time to 
		          *       base of January 1, 1900 with bit 59
		          *       being incremented each microsecond.
		          */
		         if (tz390.opt_timing){
		        	 java_mil = System.currentTimeMillis();
		     ...
		     case 0x99:  // 3300 "B299" "SRNM" "S"
		    	 ...
		    	 fp_fpc_reg = (fp_fpc_reg & fp_fpc_mask_rnd_not)
		    	              | fp_fpc_rnd; 
		         switch (fp_fpc_rnd){
		         case 0: // round half even
		        	 fp_e_context = MathContext.DECIMAL32;  // fp_fpc_rnd default half even
		        	 fp_d_context = MathContext.DECIMAL64;  // fp_fpc_rnd default half even
		        	 fp_x_context = MathContext.DECIMAL128; // fp_fpc_rnd default half even
		        	 break;
		 ...
		 case 0xB3:  
		     ...
		     case 0x38:  // 3830 "B338" "MAYLR" "RRF" Z9-4
		         ...
		         fp_rbdv1 = fp_get_bd_from_dh(fp_reg,rf2)
			      .multiply(fp_get_bd_from_dh(fp_reg,rf3),fp_db_context)
			      .add(fp_get_bd_from_dh(fp_reg,rf1))
				  .round(fp_x_context);
		         fp_put_bd(rf1,fp_lh_type,fp_rbdv1);
		         check_lh_mpy();
		         break;
		     ...
		     case 0x47:  // 3890 "B347" "FIXBR" "RRF"
			 	 psw_check = false;
			 	 ins_setup_rrf2();
	        	 fp_bd_int_rem = fp_get_bd_from_lb(fp_reg,rf2).divideAndRemainder(BigDecimal.ONE);
                 fp_put_bd(rf1,fp_lb_type,fp_get_bd_rnd(mf3));
		         break;
		     ...
		     case 0x50:  // 3960 "B350" "TBEDR" "RRF"
		     	 psw_check = false;
		         ins_setup_rrf2();
                 fp_bd_int_rem = fp_get_bd_from_dh(fp_reg,rf2).divideAndRemainder(BigDecimal.ONE);
		     ...
		     case 0x57:  // 3990 "B357" "FIEBR" "RRF"
		     	 psw_check = false;
		         ins_setup_rrf2();
                 fp_bd_int_rem = fp_get_bd_from_eb(fp_reg,rf2).divideAndRemainder(BigDecimal.ONE);
                 fp_put_eb(rf1,fp_eb_type,fp_get_bd_rnd(mf3).floatValue());
		         break;
		     ...
		     case 0x5F:  // 4030 "B35F" "FIDBR" "RRF"
		     	 psw_check = false;
		         ins_setup_rrf2();
	        	 fp_bd_int_rem = fp_get_bd_from_db(fp_reg,rf2).divideAndRemainder(BigDecimal.ONE);
                 fp_put_db(rf1,fp_db_type,fp_get_bd_rnd(mf3).doubleValue());
		         break;
		     ...
		     case 0xB8:  // 4340 "B3B8" "CFER" "RRF"
		         psw_check = false;
		     	 ins_setup_rrf2();
	        	 fp_bd_int_rem = fp_get_bd_from_eh(fp_reg,rf2).divideAndRemainder(BigDecimal.ONE);
                 rv1 = fp_get_bd_rnd(mf3).intValue(); // RPI 333
		     	 reg.putInt(rf1+4,rv1);
		     	 psw_cc = get_int_comp_cc(rv1,0);
		         break;
		     ...
		     case 0xBA:  // 4360 "B3BA" "CFXR" "RRF"
		         psw_check = false;
		     	 ins_setup_rrf2();
	        	 fp_bd_int_rem = fp_get_bd_from_lh(fp_reg,rf2).divideAndRemainder(BigDecimal.ONE);
                 rv1 = fp_get_bd_rnd(mf3).intValue(); // RPI 333
		     	 reg.putInt(rf1+4,rv1);
		     	 psw_cc = get_int_comp_cc(rv1,0);
		         break;
		 ...
		 case 0xB9:  
		     ...
		     case 0x0F:  // 4580 "B90F" "LRVGR" "RRE"
		         ...
		         rflen = 7;
		         while (rflen >= 0){
		         	reg_byte[rf1 + rflen] = reg_byte[rf2 + 7 - rflen];
		         	rflen--;
		         }
		         break;
		     ...
		     case 0x1F:  // 4730 "B91F" "LRVR" "RRE"
		         ...
		         rflen = 3;
		         while (rflen >= 0){
		         	reg_byte[rf1 + 4 + rflen] = reg_byte[rf2 + 7 - rflen];
		         	rflen--;
		         }
		         break;
		     ...
		     case 0x87:  // 4880 "B987" "DLGR" "RRE"
                 ...
                 zcvt_big_int_to_work_reg(big_int_array[1],8); // get big remainder
		         reg.putLong(rf1,work_reg.getLong(0));
                 zcvt_big_int_to_work_reg(big_int_array[0],8); // get big quotent
             	...
             	if (!trt_hit
             		&& (mem_byte[bd2_loc + (mem_byte[bd1_loc] & 0xff)])
					    != 0){
             		    ...
             		    reg_byte[r2+3] = mem_byte[bd2_loc + (mem_byte[bd1_loc] & 0xff)];
             	}
             	bd1_loc--;
             }
		     break;
		 case 0xD1:  // 5240 "D1" "MVN" "SS"
		     ...
		     ins_setup_ss();
		     while (rflen >= 8){
			    	mem.putLong(bd1_loc,(mem.getLong(bd1_loc) & long_zone_ones) | (mem.getLong(bd2_loc) & long_num_ones));
	           		bd1_loc = bd1_loc + 8;
	           		bd2_loc = bd2_loc + 8;
	           		rflen = rflen - 8;
			 }
			 if (rflen >= 4){
			    mem.putInt(bd1_loc,(mem.getInt(bd1_loc) & 0xf0f0f0f0) | (mem.getInt(bd2_loc) & 0x0f0f0f0f));
		       	bd1_loc = bd1_loc + 4;
		       	bd2_loc = bd2_loc + 4;
		       	rflen = rflen - 4;
			}
			if (rflen >= 2){
			    mem.putShort(bd1_loc,(short)((mem.getShort(bd1_loc) & (short)0xf0f0) | (mem.getShort(bd2_loc) & (short)0x0f0f)));
		       	bd1_loc = bd1_loc + 2;
		       	bd2_loc = bd2_loc + 2;
		       	rflen = rflen - 2;
			 }
		 ...
		 case 0xD3:  // 5260 "D3" "MVZ" "SS"
			 ...
			 ins_setup_ss();
		     while (rflen >= 8){
		    	mem.putLong(bd1_loc,(mem.getLong(bd1_loc) & long_num_ones) | (mem.getLong(bd2_loc) & long_zone_ones));
           		bd1_loc = bd1_loc + 8;
           		bd2_loc = bd2_loc + 8;
           		rflen = rflen - 8;
		     }
		     if (rflen >= 4){
			    mem.putInt(bd1_loc,(mem.getInt(bd1_loc) & 0x0f0f0f0f) | (mem.getInt(bd2_loc) & 0xf0f0f0f0));
	           	bd1_loc = bd1_loc + 4;
	           	bd2_loc = bd2_loc + 4;
	           	rflen = rflen - 4;
		     }
		     if (rflen >= 2){
			    mem.putShort(bd1_loc,(short)((mem.getShort(bd1_loc) & 0x0f0f) | (mem.getShort(bd2_loc) & 0xf0f0)));
	           	bd1_loc = bd1_loc + 2;
	           	bd2_loc = bd2_loc + 2;
	           	rflen = rflen - 2;
		     }
		 ...
		 case 0xD5:  // 5280 "D5" "CLC" "SS"
		     ...
		     if (rflen >= 4
	         	 && mem.getInt(bd1_loc) == mem.getInt(bd2_loc)){
	         	bd1_loc = bd1_loc + 4;
	         	bd2_loc = bd2_loc + 4;
	         	rflen = rflen - 4;
			 }
		     if (rflen >= 2
		         && mem.getShort(bd1_loc) == mem.getShort(bd2_loc)){
		         bd1_loc = bd1_loc + 2;
		         bd2_loc = bd2_loc + 2;
		         rflen = rflen - 2;
				 }
		 ...
		 case 0xE3:  
		     ...
		     case 0x0F:  // 5500 "E30F" "LRVG" "RXY"
		         ...
		         rflen = 7;
		         while (rflen >= 0){
		         	reg_byte[rf1 + rflen] = mem_byte[xbd2_loc + 7 - rflen];
		         	rflen--;
		         }
		         break;
		     ...
		     case 0x17:  // 5550 "E317" "LLGT" "RXY"
		     	 psw_check = false;
		     	 ins_setup_rxy();
		     	 reg.putLong(rf1,mem.getInt(xbd2_loc) & max_pos_int);  //RPI160 was LOC+4
		         break;
		     ...
		     case 0x1E:  // 5620 "E31E" "LRV" "RXY"
		    	 ...
		    	 rflen = 3;
		         while (rflen >= 0){
		         	reg_byte[rf1 + 4 + rflen] = mem_byte[xbd2_loc + 3 - rflen];
		         	rflen--;
		         }	         
		         break;
		     case 0x1F:  // 5630 "E31F" "LRVH" "RXY"
		         ...
		         rflen = 1;
		         while (rflen >= 0){
		         	reg_byte[rf1 + 6 + rflen] = mem_byte[xbd2_loc + 1 - rflen];
		         	rflen--;
		         }
		         break;
		     ...
		     case 0x2F:  // 5690 "E32F" "STRVG" "RXY"
		         ...
		         rflen = 7;
		         while (rflen >= 0){
		         	mem_byte[xbd2_loc + rflen] = reg_byte[rf1 + 7 - rflen];
		         	rflen--;
		         }
		         break;
		     ...
		     case 0x97:  // 6090 "E397" "DL" "RXY"
		         ...
		         big_int_array = big_int1.divideAndRemainder(big_int2);
                 zcvt_big_int_to_work_reg(big_int_array[1],4); // get big remainder
		         reg.putInt(rf1+4,work_reg.getInt(0));
                 zcvt_big_int_to_work_reg(big_int_array[0],4); // get big quotent
		         reg.putInt(rf1+12,work_reg.getInt(0));
		         break;
		 ...
		 case 0xED:  
		     ...
		     case 0x12:  // 6760 "ED12" "TCXB" "RXE"
		         ...
		         psw_cc = psw_cc0;
		         if ((xbd2_loc & 0x800) != 0 && fp_rbdv1.compareTo(BigDecimal.ZERO) == 0){
		        	 psw_cc = psw_cc1; //+0
		         }
		         if ((xbd2_loc & 0x400) != 0 && fp_rbdv1.compareTo(BigDecimal.ZERO) == 0){
		        	 psw_cc = psw_cc1; //-0
		         }
		         if ((xbd2_loc & 0x200) != 0 && fp_rbdv1.compareTo(BigDecimal.ZERO) > 0){
		        	 psw_cc = psw_cc1; //+normalized
		         }
		         if ((xbd2_loc & 0x100) != 0 && fp_rbdv1.compareTo(BigDecimal.ZERO) < 0){
		        	 psw_cc = psw_cc1; //-normalized
		         }
		         if ((xbd2_loc & 0x080) != 0){
		        	 // psw_cc = psw_cc1; //+denormalized (never)
		 ...
		 case 0xFC:  // 7120 "FC" "MP" "SS"
	         ...
	         if (pdf_is_big){
	        	 ...
	        	 if (pdf_long1 < max_pos_int
	        		 && pdf_long1 > min_neg_int
	        		 && pdf_long2 < max_pos_int
	        		 && pdf_long2 > min_neg_int){
	        		 pdf_long = pdf_long1 * pdf_long2;
	        	 } else {
	        		 pdf_is_big = true;
		 ...
		 case 0xFD:  // 7130 "FD" "DP" "SS"
	         ...
	         if (pdf_is_big){
			     ...
			     put_pd(bd1_loc,rflen1-rflen2);
			     pdf_big_int = big_quo_rem[1];
			     put_pd(bd1_loc+rflen1-rflen2,rflen2);
	         } else {
			     if (pdf_long2 == 0){
				     psw_cc = psw_cc3;
				     set_psw_check(psw_pic_pd_div);
...
private void ins_setup_rie(){  // "RIE" 4  BRXLG  oorriiii00oo
    ...
    if  (tz390.opt_trace){
        put_ins_trace(
    			    " R1" + tz390.get_hex(mf1,1) + "=" + tz390.get_hex(reg.getInt(rf1+4),8)
    			  + " R3" + tz390.get_hex(mf3,1) + "=" + tz390.get_hex(reg.getInt(rf3+4),8)
...
private void ins_setup_rr(){  // "RR" 60  LR  oorr
    ...
    if  (tz390.opt_trace){
    	if (opcode1 == 0x07){
    		rv2 = reg.getInt(rf2+4);  
     	    put_ins_trace(
                " R" + tz390.get_hex(mf2,1) + "(" + tz390.get_hex(rv2,8)
         ...
         put_ins_trace(
                ...
                + " R" + tz390.get_hex(mf2+1,1) + "=" + tz390.get_hex(reg.getInt(rf2+12),8)
				);
...
private void ins_setup_rrf1(){  // RPI 206 "RRF1" 28 MAER oooor0rr (r1,r3,r2 maps to oooo1032)
	psw_ins_len = 4;
...
private void ins_setup_rrf3(){  // RPI 206 "RRF3" 28 DIEBR/DIDBR oooor0rr (r1,r3,r2,m4 maps to oooo3412)
	psw_ins_len = 4;
...
private void ins_setup_rsy(){  // "RSY" 31  LMG  oorrbdddhhoo
    ...
    if  (tz390.opt_trace){
    	if (opcode2 == 0x20      // CLMH 
    		...
    		put_ins_trace(
  				  " R" + tz390.get_hex(mf1,1) + "=" + get_long_hex(reg.getLong(rf1))
                  + " R" + tz390.get_hex(mf3,1) + "=" + get_long_hex(reg.getLong(rf3))
  				+ " S2(" + tz390.get_hex(bd2_loc,8)
  				+ ")"
  				);
    	} else {
     	   ...
     	   put_ins_trace(
              ...
              + ")=" + get_ins_target(xbd2_loc)
   	        ...
   	        put_ins_trace(
  				  " R" + tz390.get_hex(mf1,1) + "=" + tz390.get_hex(reg.getInt(rf1+4),8)
  				+ " S2(" + tz390.get_hex(xbd2_loc & psw_amode,8)
                + ")=" + bytes_to_hex(mem_byte,xbd2_loc,2,0)
  				);
    		...
    		if (opcode1 == 0xb3 // RPI 348
    			&& (opcode2 == 0x70    				
       	        ...
       	        put_ins_trace(
        			...
        			&& (opcode2 >= 0x78 && opcode2 <= 0x7b)){
               	        put_ins_trace(
              				  " R" + tz390.get_hex(mf1,1) + "=" + tz390.get_hex(reg.getInt(rf1),8)
              			    + " S2(" + tz390.get_hex(xbd2_loc,8)
                              + ")=" + bytes_to_hex(mem_byte,xbd2_loc,2,0)
              				);
    		} else {
...
private void ins_setup_ssp(){   // AP SS2  oollbdddbddd
	 ...
	 */
		...
		if (bd1_loc + rflen1 > tot_mem 
				|| bd2_loc + rflen2 > tot_mem){ // RPI 299
				set_psw_check(psw_pic_addr);
		}
}
private void ins_setup_ss(){  // "SS" 32  MVC oollbdddbddd  
	...
	psw_loc = psw_loc + 6;
	if (bd2_loc + rflen > tot_mem 
		|| bd1_loc + rflen > tot_mem){ // RPI 299
		set_psw_check(psw_pic_addr);
	}
}
...
public String get_ins_name(int ins_loc){
	...
	hex_mf1 = Integer.toHexString((mem_byte[ins_loc+1] & 0xff) >> 4).toUpperCase();
...
public void psw_pic_handler(){
	 ...
	 *   3.  If estae_exit defined, then restart
	 *       at estae_exit.
	 *   4.  If option test on
	 ...
	 */
     ...
     if (try_espie
         && tot_espie > 0
         && !espie_exit_running  // RPI 305 prevent recursion
         && !estae_exit_running
        ){
...
private void setup_espie_exit(){
	/*
	 * initialize zcvt_epie and pass addr
	 * to espie exit via r1
	 */
...
private void setup_estae_exit(){
	/*
	 * initialize zcvt_esta and pass addr
	 * to estae exit via r1
	 */
...
private void exec_clst(){
	 ...
	 * compare strings
	 *   r0 - ending byte
	 *   rf1,rf2 - string addresses
	 */
	byte str_end_byte = reg_byte[r0+3];
...
private void exec_cuse(){
	/*
	 * search for matching string at same offset
	 * in two fields:
	 *   r0 = length up to 255
	 *   r1 = pad char
	 *   rf1,rf2 = addr and lengh regs 
	 *   cc0 - key found and rf1, rf2 updated
	 *   cc1 - ended at longer operand, last byte =
	 *   cc2 - ended at longer operand, last byte !=
	 *   cc3 - search incomplete retry 
	 */
    int  key_len  = reg.getInt(r0);
    byte str_pad  = reg_byte[r1+3];
    int  str1_loc = reg.getInt(rf1+4) & psw_amode;
...
private void exec_srst(){
	/*
	 * find char in r2 field which ends at r1
	 * char is in r0
	 *  cc 1 = char found and r1 set to address
	 *  cc 2 = char not found, no update
	 *  cc 3 = instruction incomplete retry
	 */
	byte key_byte = reg_byte[r0+3];
...
private void exec_icm(){
	 ...
	 * Notes:
	 *   1.  psw_cc = allinserted bits 0 or mask 0
...
private void pop_pc_stack(){
	...
	if (cur_pc_stk > 0){
		...
		reg.position(8);
		reg.put(pc_stk_reg_byte,cur_pc_stk_reg+8,reg_byte.length-16);
	} else {
		set_psw_check(psw_pic_stkerr);
	}
}
...
private String trace_svc(){ // RPI 312
	/*
	 * return extended svc trace information
	 * including svc function name and any
	 * key parms available such as pgm name
	 * or ddname where appropriate
	 */
	switch (if1){
	case 0x01:  // WAIT
	...
	case 0x06:  // LINK  R0=PGMID, R1=PARMS
        return "LINK R1=PARMS R0=PGM(" + sz390.get_ascii_string(reg.getInt(r0) & psw_amode,8).toUpperCase() + ")";
	case 0x07:  // XCTL  R15=PGMID, R1=PARMS
		return "XCTL R1=PARMS R0=PGM(" + sz390.get_ascii_string(reg.getInt(r0) & psw_amode,8).toUpperCase() + ")";
	case 0x08:  // LOAD  R15=PGMID
		return "LOAD R1=PARMS R0=PGM(" + sz390.get_ascii_string(reg.getInt(r0) & psw_amode,8).toUpperCase() + ")";
	case 0x09: // DELETE R0=A(NAME)
		return "DELETE R1=PARMS R0=PGM(" + sz390.get_ascii_string(reg.getInt(r0) & psw_amode,8).toUpperCase() + ")";
	...
	case 0x6d:
		return "ESPIE R0=TYPE INT, R1=EXIT ADDR";  // set program check exit R0=types, R1=addr
	...
	case 93:  // TGET/TPUT TN3290 data stream for GUI
		...
		if (flags >= 0x80){
				return "TGET" 
				     + " FLAGS=" + tz390.get_hex(flags,2)
				     + " LEN="   + tz390.get_hex(reg.getShort(r0+2),2)
				     + " ADDR="  + tz390.get_hex(addr,6);
		} else {
			return "TPUT" 
		     + " FLAGS=" + tz390.get_hex(flags,2)
		     + " LEN="   + tz390.get_hex(reg.getShort(r0+2),2)
		    ...
		    fp_dxc = fp_dxc_oe;
 			set_psw_check(psw_pic_data);
 				  ...
 				  || (bd2.abs().compareTo(fp_lb_min) <= 0
 					  && !(bd2.abs().compareTo(BigDecimal.ZERO) == 0))
 				  ){
		    fp_dxc = fp_dxc_ue;
 			set_psw_check(psw_pic_data);
 		}
...
private BigDecimal fp_get_bd_rnd(int mode){
	...
	case 4: // round to nearest with default half even
		if (rem_comp == 0){
			if (fp_bd_int_rem[0].remainder(fp_bd_two).compareTo(BigDecimal.ZERO) == 0){
				return fp_bd_int_rem[0]; 
			} else {
				return fp_bd_int_rem[0].add(fp_bd_inc); 
			}
...
private void fp_put_db(int fp_index,byte reg_type,double reg_value){
	/*
	 * store db co-reg as EH, DH, or DB
	 * and set type (EH used double due to
...
private void fp_put_bd(int fp_index,byte reg_type,BigDecimal reg_value){
	/*
	 * store bd co-reg as LH or LB
	 * and set type for reg pair
	 */
	int fp_ctl_index = fp_index >> 3;
...
public void fp_store_reg(ByteBuffer reg_buff,int reg_index){
	...
	case 2: // fp_ctl_db
		...
		case 0: // fp_db_type
			...
			break;
		case 1: // fp_dh_type
			reg_buff.putLong(reg_index,zcvt_db_to_dh(fp_reg_db[fp_ctl_index]));
			break;
		case 3: // fp_eh_type
			reg_buff.putInt(reg_index,zcvt_db_to_eh(fp_reg_db[fp_ctl_index]));
			break;
		}
		break;
	case 3: // fp_ctl_bd1
 		...
 		case 4: // fp_lb_type
 			if (fp_reg_ctl[fp_ctl_index+2] == fp_ctl_bd2){
 				zcvt_bd(fp_lb_type,fp_reg_bd[fp_ctl_index]);
 				reg_buff.putLong(reg_index,fp_work_reg.getLong(0));
 			}
		    break;
 		case 5: // fp_lh_type
 			if (fp_reg_ctl[fp_ctl_index+2] == fp_ctl_bd2){
 				zcvt_bd(fp_lh_type,fp_reg_bd[fp_ctl_index]);
 				reg_buff.putLong(reg_index,fp_work_reg.getLong(0));
 			}   
 		 	break;
 		}
    ...
    case 4: // fp_ctl_bd2
 		...
 		case 4: // fp_lb_type
 			if (fp_reg_ctl[fp_ctl_index-2] == fp_ctl_bd1){
 				zcvt_bd(fp_lb_type,fp_reg_bd[fp_ctl_index-2]);  // RPI 229
 				reg_buff.putLong(reg_index,fp_work_reg.getLong(8));
 			}
 		 	break;
 		case 5: // fp_lh_type     
 			if (fp_reg_ctl[fp_ctl_index-2] == fp_ctl_bd1){
 				zcvt_bd(fp_lh_type,fp_reg_bd[fp_ctl_index-2]); // RPI 229
 				reg_buff.putLong(reg_index,fp_work_reg.getLong(8));
 			}  
 		 	break;
 		}
...
private BigDecimal zcvt_eh_to_bd(ByteBuffer eh1_buff, int eh1_index){
	 ...
	 * 
	 */
	return new BigDecimal(zcvt_eh_to_db(eh1_buff.getInt(eh1_index)),fp_e_context);
}
private BigDecimal zcvt_dh_to_bd(ByteBuffer dh1_buff, int dh1_index){
	 ...
	 */
	return new BigDecimal(zcvt_dh_to_db(dh1_buff.getLong(dh1_index)),fp_d_context);
}
...
private void get_pdf_ints(){
	/*
	 * set pdf_is_big to true or false and set
	 * pdf_big_int1 and pdf_big_int2 or
	 * pdf_long1 and pdf_long2
	 */
    if (get_pd(bd1_loc,rflen1)){ // RPI 305
    	if (pdf_is_big){
...
private boolean get_pd(int pdf_loc,int pdf_len){
	/*
	 * convert pd field to pdf_big_int 
	 * or convert to pdf_long and return true 
	 * and set pd_cc for use by TP as follows:
	 *   cc0 - sign and digits ok
	 *   cc1 = sign invalid
	 *   cc2 = at least one digit invalid
	 *   cc3 = sign and at least one digit invalid
	 * else return false if error causing 0C7
	 * 
	 * Notes:
...
private void put_pd(int pdf_loc,int pdf_len){
	/*
	 * if pdf_is_big
	 *    store pdf_big_int value into pd field
	 *    and set pd_cc
	 * else 
	 *    store pdf_long into pdf field
	 *    and set pd_cc.
	 */
	if (pdf_is_big){
	...
	while (index >= end_index
			&& pdf_index >= pdf_loc){
		if (left_digit){
			mem_byte[pdf_index] = (byte)(next_byte | ((pdf_str.charAt(index) & 0xf) << 4));
			next_byte = 0;
			pdf_index--;
			left_digit = false;
		} else {
...
public String bytes_to_hex(byte[] bytes,int byte_start,int byte_length,int chunk){
   	/*
   	 ...
   	 */
        ...
        if (byte_start + byte_length > bytes.length){
        	byte_length = bytes.length - byte_start;
        }
...
private void init_mem(){
	 ...
	 * 2.  Set zcvt_ipl_pgm to name of 
	 *     opt_ipl pgm name if any to run via
	 *     link svc 6.
	 * 3.  Set zcvt_user_pgm to name of user
	 *     initial program to run via link svc.
	 * 4.  Set mem_fqe_ptr to point to free queue
	 *     with all of available memory for use by 
	 *     svc 4 getmain and svc 5 freemain.
	 *     a.  MEM(MB) defaults to 1 MB but can be
	 *         set to any value up to available
	 *         memory on machine.  See TEST\TESTMEM1
	 *         for test of ABOVE/BELOW GETMAIN using
	 *         MEM(32) to allocate 16MB below and 
	 *         16 MB above.
	 *     b.  If less than 16 MB allocated 31 bit
	 *         fqe is set to 0 and RMODE31 requests
	 *         use memory below the line.
	 * 5.  Add 8 bytes to physical mem_byte array 
	 *     allocated to allow get_pd to fetch 8
	 *     bytes from last byte in memory for 1-8
	 *     byte packed decimal field.
	 * Notes:
	 *   1.  The default J2RE maximum memory available is
	 *       limited by default to small fraction of total
	 *       memory to prevent stalling OS or other
	 *       J2RE applications running.  To override
	 *       default set -Xmx option at startup of
	 *       J2RE.  For example, add -Xmx500000000
	 *       to z390.bat to allow allocating up to 
	 *       500 MB but don't allocate all of it
	 *       or other J2RE application such as IE
	 *       may stall or abort.
	 */
	...
	/*
	 * init zcvt_exec_parm from PARM(..) option
	 * Notes:
	 *   1.  ez390 sets R1 to zcvt_exec_parm at start
	 */
