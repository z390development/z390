...
clrnum = popclrnum(document.getElementById(posub[x-p3+p4][0]).parentNode.parentNode);
...
top = smpy;
left = smpx;
zIndex = eval(trgtlay.style.zIndex)+1;
visibility = "visible";
}
}
...
if (deletehilay){
clrnum = popclrnum(document.getElementById(id).parentNode.parentNode);
...
function popclrnum(obj){
if (obj.id==null)
obj = obj.parentNode;
clrnum = obj.id.replace("submain","");
clrnum = clrnum.replace("pop","");
clrnum = (clrnum.indexOf("Main")<0) ? popclrindx[clrnum] : popclrindx[0];
return clrnum;
}
...
//Pop-Up 4.7 H Menu file (Konqueror)
...
ccf4[x] += ';width:';
ccf5[x] = '" onmouseover="popcacnelso()" style="position: relative;visibility: inherit;cursor: default;';
if (pmclr[popclrindx[x]][15]==0)
ccf5[x] += 'top:2px;left:3px;width:99%;padding:2px 5px 4px 0px';
else
if (pmclr[popclrindx[x]][15]==1)
ccf5[x] += 'top:2px;left:0px;width:100%;padding:2px 0px 4px 0px';
else
ccf5[x] += 'top:0px;left:0px;width:101%;margin-bottom:1px';
ccf5[x] += '"';
}
...
pmhcde='<Div Id="popMain" style="position:absolute;left:'+poX+';top:'+(poY)+';background-color:'+pmclr[0][0]+'"><TABLE Id="popMaini" cellpadding=0 cellspacing=0 width='+popmcwidth+' height='+popmcheight+'><TR Id="popHtr" style="cursor:default'+pmcfnt+'">';
